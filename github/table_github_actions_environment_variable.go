...
func tableGitHubActionsEnvironmentVariable() *plugin.Table {
	return &plugin.Table{
		...
		Description: "Variables are unencrypted environment variables that you create in a repository environment for use in GitHub Actions workflows.",
		List: &plugin.ListConfig{
			KeyColumns:        plugin.AllColumns([]string{"repository_full_name", "environment_name"}),
			ShouldIgnoreError: isNotFoundError([]string{"404"}),
			Hydrate:           tableGitHubEnvVariableList,
		},
		Get: &plugin.GetConfig{
			KeyColumns:        plugin.AllColumns([]string{"repository_full_name", "environment_name", "name"}),
			ShouldIgnoreError: isNotFoundError([]string{"404"}),
			Hydrate:           tableGitHubEnvVariableGet,
		},
		Columns: commonColumns([]*plugin.Column{
			{Name: "repository_full_name", Type: proto.ColumnType_STRING, Transform: transform.FromQual("repository_full_name"), Description: "Full name of the repository that contains the environment."},
			{Name: "environment_name", Type: proto.ColumnType_STRING, Transform: transform.FromQual("environment_name"), Description: "The name of the environment."},
...
func tableGitHubEnvVariableList(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	client := connect(ctx, d)

	repoFullName := d.EqualsQuals["repository_full_name"].GetStringValue()
	environmentName := d.EqualsQuals["environment_name"].GetStringValue()
...
func tableGitHubEnvVariableGet(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	name := d.EqualsQuals["name"].GetStringValue()
	repoFullName := d.EqualsQuals["repository_full_name"].GetStringValue()
	environmentName := d.EqualsQuals["environment_name"].GetStringValue()

	// Empty check for the parameters
	if name == "" || repoFullName == "" || environmentName == "" {
		return nil, nil
	}

	owner, repo := parseRepoFullName(repoFullName)
	plugin.Logger(ctx).Trace("tableGitHubEnvVariableGet", "owner", owner, "repo", repo, "environment", environmentName, "name", name)
