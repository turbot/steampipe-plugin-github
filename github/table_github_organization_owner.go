...
func gitHubOrganizationOwnerColumns() []*plugin.Column {
	ownerCols := []*plugin.Column{
		...
		{Name: "members_can_fork_private_repositories", Type: proto.ColumnType_STRING, Transform: transform.FromField("MembersCanForkPrivateRepositories", "Node.MembersCanForkPrivateRepositories"), Description: "If true, members can fork private repositories in this organization."},
		...
		{Name: "requires_two_factor_authentication", Type: proto.ColumnType_STRING, Transform: transform.FromField("RequiresTwoFactorAuthentication", "Node.RequiresTwoFactorAuthentication"), Description: "If true, the organization requires all members, billing managers, and outside collaborators to enable two-factor authentication."},
		{Name: "web_commit_signoff_required", Type: proto.ColumnType_STRING, Transform: transform.FromField("WebCommitSignoffRequired", "Node.WebCommitSignoffRequired"), Description: "Whether contributors are required to sign off on web-based commits for repositories in this organization."},
	}
